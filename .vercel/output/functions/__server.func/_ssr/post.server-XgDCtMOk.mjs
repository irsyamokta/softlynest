...
//#region node_modules/.nitro/vite/services/ssr/assets/post.server-XgDCtMOk.js
...
var uploadMedia = async (base64OrUrl, folder = "softlynest") => {
	try {
		return await import_cloudinary.v2.uploader.upload(base64OrUrl, {
			folder,
			resource_type: "auto"
		});
	} catch (error) {
...
var deleteMedia = async (publicId, resourceType = "image") => {
	try {
		return await import_cloudinary.v2.uploader.destroy(publicId, { resource_type: resourceType });
	} catch (error) {
...
var getPublicIdFromUrl = (url) => {
	...
	if (remainingParts.length > 1 && /^v\d+$/.test(remainingParts[0])) remainingParts.shift();
...
var getUserPostsFn = createServerFn({ method: "POST" }).validator((data) => data).handler(getUserPostsFn_createServerFn_handler, async ({ data }) => {
	...
	return (await prisma.post.findMany({
		where: {
			...data.targetUserId ? { userId: data.targetUserId } : { user: { username: data.targetUsername } },
...
var deletePostFn = createServerFn({ method: "POST" }).validator((data) => data).handler(deletePostFn_createServerFn_handler, async ({ data }) => {
	...
	if (post.image) {
		...
		if (publicId) await deleteMedia(publicId, "image").catch((err) => {
			console.error("Failed to delete post image from Cloudinary:", err);
		});
	}
	if (post.video) {
		const publicId = getPublicIdFromUrl(post.video);
		if (publicId) await deleteMedia(publicId, "video").catch((err) => {
